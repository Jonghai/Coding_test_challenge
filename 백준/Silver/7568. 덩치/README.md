...
## 문제 이해
사람의 키와 몸무게를 등수를 매기는 문제인데 이번 문제는 되게 길지만 요약하자면 핵심은 딱 3가지. <br>
Ex) A의 키: 177, 몸무게: 56, B의 키: 165, 몸무게: 45 일 때
1.  **키도 몸무게도 둘 다 큰 경우 -> A가 B보다 덩치가 있다.**
<br>
Ex) C의 키: 181, 몸무게: 45, D의 키: 173, 몸무게: 55 일 때
2. **키나 몸무게 둘 중 하나만 상대보다 큰 경우 -> 공동 등수**

Ex) E의 키: 155, 몸무게: 46
3.  **키와 몸무게 모두 상대들에 비해 작은 경우 -> 덩치가 작다.**

2번 케이스의 경우
공동 등수를 가지게 되므로 총 5명 중 3명이 공동 2등이라면
1등 1명, 2등 3명, (3,4등이 존재하지않고), 5등 1명
이렇게 츨력이 되어야 한다.

## 코드 설계
- 첫 줄: 사람 수 `n`
- 이후 `n`줄: 각 사람의 몸무게와 키를 입력받아 `List<Person>`에 저장

- 각 사람마다 (`i`) 다른 모든 사람 (`j`)을 비교
- 나보다 키와 몸무게가 모두 큰 사람이 있으면 `rank++`

- 계산된 `rank`를 공백으로 구분하여 한 줄로 출력


### 추가 사항항
- **공동 등수 처리**는 `rank`로 자동 해결
- `for (i=0; i<n)` / `for (j=0; j<n)` 이중 반복으로 전체 비교
