...
## 문제 이해
1. N명의 학생 중 점수가 가장 높은 K명은 상을 받는다.
2. 상을 받는 커트라인 점수 구하기.
3. 커트라인이랑 상을 받는 사람들 중 점수가 가장 낮은 사람의 점수.
4.  첫 줄에 응시자 수: N, 상을 받을 사람의 수 K 공백을 두고 주어짐.
5. 둘 째 줄에는 각 학생의 점수 x가 공백을 사이에 두고 주어짐.

### 정리
x개의 점수들 중 K 번째 높은 점수 구하기
ex) 5명의 학생 중 2명이 상을 받는다면 응시자들 중 2번째로 높은 2등 점수 구하기

## 코드 설계
1. **학생 수(N)** 와 **상 줄 인원 수(K)** 를 입력 받음.
2. N명의 점수를 공백으로 입력받아 **정수 리스트(scores)** 에 저장.
3. **내림차순**으로 정렬하여 **가장 높은 점수부터 정렬**함.
4. 상을 받을 사람 중 **가장 낮은 점수**는 K등 점수 → 인덱스는 `K - 1`
5. 해당 점수를 출력.
